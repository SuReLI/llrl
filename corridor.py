...
def experiment():
    ...
    lrmaxprior01 = LRMax(actions=actions, gamma=gamma, r_max=r_max, v_max=v_max, deduce_v_max=False, n_known=n_known,
                         deduce_n_known=False, epsilon_q=epsilon_q, epsilon_m=epsilon_m, delta=delta, n_states=n_states,
                         max_memory_size=max_mem, prior=0.1, estimate_distances_online=False,
                         min_sampling_probability=p_min, name='LRMax(Dmax0.1)')
    ...
    agents_pool = [rmax, lrmax, lrmaxprior01, lrmaxprior02, maxqinit, lrmaxqinit, lrmaxqinitprior02] # TODO put back

    # Run
    # TODO maybe put n_instances=5 back
    run_agents_lifelong(agents_pool, env_distribution, name_identifier=None, n_instances=1, n_tasks=20, n_episodes=20,
                        n_steps=11, reset_at_terminal=False, plot_only=False, open_plot=True, plot_title=True)
