...
def print_upper_bound(bound):
    ...
    for s in bound:
        for a in bound[s]:
            print('{:>17}  {:>17}  {:>17}'.format(str(s), a, bound[s][a]))
        ...
        for i in range(self.vi_n_iter):
            ...
            for s in self.R:
                for a in self.R[s]:
                    ...
                    buffer[s][a] = r_s_a + self.gamma * weighted_next_upper_bound
            for s in self.U:
                for a in self.U[s]:
                    self.U[s][a] = buffer[s][a]
        ''''''
