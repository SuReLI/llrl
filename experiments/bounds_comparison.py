...
def main():
    # MDP
    size = 5
    mdp1 = GridWorldMDP(width=size, height=size, init_loc=(1, 1), goal_locs=[(size, size)], slip_prob=0.00)
    mdp2 = GridWorldMDP(width=size, height=size, init_loc=(1, 1), goal_locs=[(size, size)], slip_prob=0.01)
    # mdp1 = TwoStates(rewards=(0., 1.), proba=(0., 0.))
    # mdp2 = TwoStates(rewards=(0., 1.), proba=(.9, .9))

    # Agents
    rand_agent = RandomAgent(actions=mdp1.get_actions())
    rmax_vi_agent = RMaxVI(actions=mdp1.get_actions(), gamma=.9, count_threshold=10)
    lrmax_agent = LRMax(actions=mdp1.get_actions(), gamma=.9, count_threshold=10)
    ...
    run_agents_on_mdp([lrmax_agent, rmax_vi_agent, rand_agent], mdp2, instances=1, episodes=100, steps=30,
                      reset_at_terminal=True, verbose=False, open_plot=False)
